...

{
	...
	fstream infile2; //to read index set if exists

	ofstream outfile; //to write to index set

	ifstream newrecord_file;
	
	ofstream newrecord_out; // to populate our record file after insertion 
	
	ofstream newindex_out; //to populate the index set after insertion
...

		{
...

			{
				...
				case'1':
					...
					//b.Display(b,outfile); //prints new sequence set
				...
				case'2':
					...
					infile1.close();
					newrecord_out.open(filename,std::ofstream::app); //was trying to open the previous record file to write on it
					newrecord_out<<endl;
					infile2.close();
					...
					while(!newrecord_file.eof())
					{
						...
						newrecord_out<<record<<endl;	//updating our record file
